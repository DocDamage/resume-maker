...
export function TemplateModern({ resume }: { resume: Resume }) {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div style={{ marginTop: "20px" }}>
      <h2 style={{ fontSize: "14px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1px", color: accent, borderBottom: `2px solid ${accent}`, paddingBottom: "4px", marginBottom: "10px", fontFamily }}>{title}</h2>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ fontFamily, color: textColor, lineHeight: "inherit", fontSize: "13px" }}>
      <div style={{ textAlign: "center", marginBottom: "16px" }}>
        {photoUrl && <img src={photoUrl} alt="" style={{ width: 80, height: 80, borderRadius: "50%", objectFit: "cover", marginBottom: "8px", border: `3px solid ${accent}` }} />}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "12px", marginTop: "8px", fontSize: "12px", color: mutedColor }}>
      ...
      {visible("summary") && summary && <Section title="Summary"><p style={{ color: textColor, fontSize: "13px" }}>{summary}</p></Section>}
        ...
        <Section title="Certifications">
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            {certifications.map((cert) => (
              <div key={cert.id}>
                <strong style={{ fontSize: "12px", color: strongColor }}>{cert.name}</strong>
                <span style={{ fontSize: "11px", color: mutedColor }}> — {cert.issuer}{cert.date ? `, ${cert.date}` : ""}</span>
              </div>
            ))}
          </div>
        </Section>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            {references.map((ref) => (
              <div key={ref.id}>
                ...
                <span style={{ fontSize: "11px", color: mutedColor }}> | {ref.email}{ref.phone ? ` | ${ref.phone}` : ""}</span>
              </div>
            ))}
          </div>
        </Section>
      ...
      {customSections.map((cs) => (
        visible(cs.id) && cs.items.length > 0 && (
