...
export function TemplateClassic({ resume }: { resume: Resume }) {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div style={{ marginTop: "18px" }}>
      <h2 style={{ fontSize: "15px", fontWeight: 700, color: "#0f172a", borderBottom: "1px solid #cbd5e1", paddingBottom: "4px", marginBottom: "10px", fontFamily }}>{title}</h2>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ fontFamily, color: "#1e293b", lineHeight: "inherit", fontSize: "13px" }}>
      <div style={{ marginBottom: "16px", display: "flex", alignItems: "center", gap: "16px" }}>
        ...
        <div>
          <h1 style={{ fontSize: "28px", fontWeight: 700, color: "#0f172a", margin: 0, fontFamily }}>{personal.fullName}</h1>
          <p style={{ fontSize: "14px", fontWeight: 500, color: "#475569", marginTop: "4px", fontFamily }}>{personal.title}</p>
        </div>
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginBottom: "16px", fontSize: "12px", color: "#64748b" }}>
      ...
      {summary && <Section title="Professional Summary"><p style={{ color: "#334155", fontSize: "13px" }}>{summary}</p></Section>}
            ...
            {experience.map((exp) => (
              <div key={exp.id}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <strong style={{ fontSize: "13px", color: "#0f172a" }}>{exp.company}</strong>
