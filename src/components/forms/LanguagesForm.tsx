  ...
  return (
    <div ref={setNodeRef} style={style} className="border rounded-lg p-4 bg-card space-y-3">
      <div className="flex items-start gap-2">
        ...
        <Button variant="ghost" size="icon" className="text-destructive shrink-0 hover:bg-destructive/10" onClick={() => removeLanguage(lang.id)}>
          <Trash2 size={16} />
        </Button>
      </div>
    </div>
      ...
      <CardContent className="space-y-4">
        {languages.length === 0 && (
          <div className="text-center py-8 border-2 border-dashed border-border rounded-lg">
            ...
            <p className="text-sm font-medium text-muted-foreground">No languages yet</p>
            <p className="text-xs text-muted-foreground mt-1">Add languages you speak and your proficiency level</p>
            <Button size="sm" variant="outline" className="mt-3" onClick={addLanguage}>
